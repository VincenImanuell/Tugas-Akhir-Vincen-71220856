...
def read_grid_search(path):
    """Ambil baris rank 1 (terpilih) dari grid search fase 1 dan fase 2."""
    stem = os.path.splitext(os.path.basename(path))[0]
    results = {}
    for sheet in ["Grid Search Train Z+SLTP", "Grid Search Train RSI"]:
        try:
            df = pd.read_excel(path, sheet_name=sheet)
            top = df.iloc[[0]].copy()   # rank 1 = baris pertama (sudah diurutkan Sharpe)
...
for row_data in df_rq1.values.tolist():
    ws1.append(row_data)
ws1_header_row = ws1.max_row - len(df_rq1)
...
cols_rq4 = ["Pair","Sektor","Metode","Y","X",
            ...
            "BH_Y(%)","BH_X(%)","Base_vs_BH_Y(%)","RSI_vs_BH_Y(%)"]

df_rq4 = df_all[["Pair","Sektor","Metode","Y","X",
                   "Base_Return","Base_Sharpe","Base_MDD","Base_WinRate","Base_Trades",
                   "RSI_Return","RSI_Sharpe","RSI_MDD","RSI_WinRate","RSI_Trades",
                   "BH_Y","BH_X","Base_vs_BH_Y","RSI_vs_BH_Y"]].copy()
...
# ─── Sheet Grid Search Rank-1 Fase 1 ─────────────────────────────────────────
if not df_gs_base.empty:
    ws5 = wb.create_sheet("GS Fase1 - Terpilih (Train)")
    write_df_to_sheet(ws5, df_gs_base,
                      title="Grid Search Fase 1 — Baris Terpilih (Rank 1) per Pair, Data Training")

# ─── Sheet Grid Search Rank-1 Fase 2 ─────────────────────────────────────────
if not df_gs_rsi.empty:
    ws6 = wb.create_sheet("GS Fase2 RSI - Terpilih (Train)")
    write_df_to_sheet(ws6, df_gs_rsi,
                      title="Grid Search Fase 2 RSI — Baris Terpilih (Rank 1) per Pair, Data Training")
